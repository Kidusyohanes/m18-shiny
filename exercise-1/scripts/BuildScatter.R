...
library(ggplot2)
BuildScatter <- function(data,xVar,yVar,clr,title='Title',xlab='X Title',ylab='Y Title') {
  p <- ggplot(data=data)+
      geom_point(mapping = aes(x=data[,xVar],y=data[,yVar],color=data[,clr])) + labs(title=title,x=xlab, y=ylab)
  return(p)
}
